...
- (void)makeToastActivity:(id)position {
    ...
    activityView.backgroundColor = [ColorPink colorWithAlphaComponent:CSToastOpacity];
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2 + kTabbarHeight + kAdbarHeight)) - CSToastVerticalPadding);
...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    wrapperView.backgroundColor = [ColorPink colorWithAlphaComponent:CSToastOpacity];
